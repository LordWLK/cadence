      ...
      {results.length > 0 && (
        <div className="space-y-1 max-h-48 overflow-y-auto">
          {results.filter(r => !savedIds.has(r.id)).slice(0, 8).map((item) => (
            ...
            >
              {item.imageUrl && (
                <img src={item.imageUrl} alt="" className="w-6 h-6 rounded object-contain bg-white/5" />
              )}
        ...
        <div className="space-y-1">
          ...
          <div className="flex flex-wrap gap-2">
            {saved.map((pref) => (
              ...
              >
                {pref.entity_name}
                <button onClick={() => onRemove(pref.id)} className="text-text-dim hover:text-error transition-colors">
                  <X size={12} />
                </button>
              </span>
            ))}
